...
class GeneticAlgorithm:
	...
	def findOptimum(self):
		for i in range(self.iterationLimit):
			...
			l = []
			l.extend(self.BestCombinations(self.bestCombo))
			for i in range(6):
				index1, index2 = random.sample(domain, 2)
				parent9, parent10 =  self.crossover(self.bestCombo[index1], self.bestCombo[index2])
				self.mutate(parent9)
