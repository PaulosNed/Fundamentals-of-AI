class Node:
    ...
    def add_neighbour(self, nameOfEdge, weightOfEdge=0, neighbour):
        if not isinstance(neighbour, Node):
            neighbour = Node(neighbour)
        self.neighbourList.append(neighbour.name)
        a = Edge(nameOfEdge, self, neighbour, weightOfEdge)
        self.edgeList.append(a.name)
