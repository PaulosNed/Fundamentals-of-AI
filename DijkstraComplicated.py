    ...
    def dijkstra(self, startingNode, targetNode):
        ...
        while (len(visited) < len(self.nodes)):
            ...
            for visitedNode in visited:
                for neighbour in self.neighbouringNodes[visitedNode]:
                    if neighbour in visited:
                        continue
                    temp = (visitedNode, neighbour)
                    if temp not in self.edgesOfNode.keys():
                        temp = (neighbour, visitedNode)

                    if (neighbour not in tempDis.keys()) or tempDis[neighbour] > (distance[visitedNode] + self.edgesOfNode[temp].weight):
                        tempDis[neighbour] = distance[visitedNode]+self.edgesOfNode[temp].weight
                        tempAdj[neighbour] = visitedNode

            minKey = min(tempDis, key=tempDis.get)
            visited.append(minKey)
            distance[minKey] = tempDis[minKey]
            adjacentsList[minKey] = tempAdj[minKey]
